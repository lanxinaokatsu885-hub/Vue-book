...
- **问题描述**：登录页新增的浮动 SVG 装饰在前端显示时主体图案不明显，看起来只有线条轮廓。
- **根本原因**：SVG 根节点和多数路径使用 `fill="none"` 或仅配置低透明度描边，缺少可见的填充层次；同时装饰整体透明度偏低，被登录页背景图和渐变遮罩压淡。
- **解决方案**：将浮动装饰改为带径向光晕、渐变填充书页和辅助光点的 SVG 结构，并提升装饰透明度、补充层级控制，确保图案既可见又不遮挡登录文案。
- **修改文件**：`client/src/views/LoginView.vue`、`client/src/styles.css`

### 1.0.1 登录页品牌图片不显示

- **日期**：2026-06-07
- **问题描述**：登录窗口左上角品牌图片显示为破图图标。
- **根本原因**：登录页使用 `logoSrc = '/1.jpg'`，但 Vite 开发环境没有提供该图片路径；同时项目计划移除原有 `1.jpg`、`3.jpg` 图片资源。
- **解决方案**：将品牌图替换为内联书本 SVG，登录页背景和用户端背景改为 CSS 渐变装饰，并逐个删除 `public/1.jpg`、`public/3.jpg`。
- **修改文件**：`client/src/views/LoginView.vue`、`client/src/styles.css`、`public/1.jpg`、`public/3.jpg`
...
- **日期**：2026-06-07
- **问题描述**：访问 `http://localhost:5173/login` 时页面可以打开，但点击登录后无法进入系统。
- **根本原因**：本地只启动了 Vite 前端服务，后端 `http://127.0.0.1:3000` 未监听，导致 `/api/login` 代理目标不可用。
- **解决方案**：启动后端 `node server.js`，确认日志出现“数据库连接成功”和“服务器运行在 http://0.0.0.0:3000”；再通过 `http://127.0.0.1:5173/api/list-weeks` 验证前端代理可访问后端，并在前端 API 层补充后端连接失败提示。
- **修改文件**：`client/src/services/api.js`、`mixed.md`、`Ops.md`
